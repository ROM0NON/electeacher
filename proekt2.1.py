...
class TestEditor:
    def __init__(self, root):
        self.root = root
        self.root.title("Электронный учитель v2")
    ...
    def update_questions_list(self):
        ...
        for i, question in enumerate(self.test_data["questions"], 1):
            ...
            if self.user_mode == "teacher":
                ...
                self.user_answers.append(StringVar(value="-1"))
    ...
    def save_results(self):
        ...
        results = {
            ...
            "student_name": self.simpledialog.askstring("Результаты", "Введите ваше имя:"),
            "date": datetime.now().strftime("%Y-%m-%d %H:%M"),
            "answers": [],
            "score": 0
        }
        
        if not results["student_name"]:
        ...
        filepath = filedialog.asksaveasfilename(
            defaultextension=".json",
            filetypes=[("JSON файлы", "*.json")],
            initialfile=f"results_{results['student_name']}.json"
        )
